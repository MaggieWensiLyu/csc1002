...
def validDirection():                                 
    choices = []                                      
    i = G.index(size * size)
    if  i % size !=0:                                     #whether can choose left
        choices.append("left-" + l + " ")
    if (i+1) % size != 0:                                 #whether can choose right
        choices.append("right-" + r +" ")
    if i >= size:                                            #whether can choose up
        choices.append("up-" + u + " ")
    if i < (size-1)*size:                                    #whether can choose down
        choices.append("down-" + d + " ")
    y = str(choices)
    x = "next step(" + y + "):"
    dir = input(x)
    return dir    
